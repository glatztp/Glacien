...
const useResponsiveDimension = (
  ...
  useEffect(() => {
    ...
    const calc = () => {
      ...
      if (w <= 480 && configWithResponsive["mobile" + k])
        v = configWithResponsive["mobile" + k];
      else if (w <= 768 && configWithResponsive["tablet" + k])
        v = configWithResponsive["tablet" + k];
      else if (w <= 1024 && configWithResponsive["desktop" + k])
        v = configWithResponsive["desktop" + k];
      setVal(v);
    };
  ...
  return responsive ? val : (config[key] as string);
};
...
// Attach static properties with proper typing
const GradualBlurWithStatics = GradualBlurMemo as typeof GradualBlurMemo & {
  PRESETS: typeof PRESETS;
  CURVE_FUNCTIONS: typeof CURVE_FUNCTIONS;
};

GradualBlurWithStatics.PRESETS = PRESETS;
GradualBlurWithStatics.CURVE_FUNCTIONS = CURVE_FUNCTIONS;

export default GradualBlurWithStatics;
